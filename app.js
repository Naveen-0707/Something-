...
(() => {
  ...
  function renderList(query = "") {
    ...
    for (const n of items) {
      ...
      const meta = `${extractLinks(n.body).length} links · ${timeAgo(n.updated)}`;
      div.innerHTML = `<div class="ni-title">${highlight(title, q)}</div><div class="ni-meta">${meta}</div>`;
  ...
  function seed() {
    ...
    const samples = [
      ["Welcome to your Second Brain",
        "# Welcome 👋\n\nThis is your **second brain** — a place to capture ideas and connect them.\n\n## How linking works\nWrite a link to any note with double brackets: [[Zettelkasten]].\nIf the note doesn't exist yet, clicking the link creates it.\n\nTry opening the [[Graph View]] to see how everything connects.\n\n## Markdown\n- **bold**, *italic*, `code`\n- > blockquotes\n- lists like this one\n\nSee also: [[Tips & Shortcuts]]"],
      ["Zettelkasten",
